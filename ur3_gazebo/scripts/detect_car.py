...
import ur3_gazebo.process_image as proc
...
class DetectCar:
    ...
    def __init__(self):
        ...
        self.get_logger().info('Looking for the Car...')
        self.image_sub = rospy.Subscriber(Image,"/camera/image_raw",Image,queue_size=1)
        self.image_out_pub = rospy.Publisher(Image, "/image/mask", Image,queue_size=1)
        self.image_tuning_pub = rospy.Publisher(Image, "/image/image_tuning", Image,queue_size=1)
        self.ball_pub  = rospy.Publisher(Point, "/car/position", Point,queue_size=1)
...
def main(args=None):
    ...
    rospy.init_node('detect_car', anonymous=True)
